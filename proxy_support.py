...
def send_message(token, channel_id, text, antispam):
    ...
    payload = {'content':text, 
     'tts':False}
    src = request.post(f"https://canary.discordapp.com/api/v6/channels/{channel_id}/messages", headers=headers, json=payload, timeout=10, proxies={"http" : "http://" + random.choice(proxies)})
...
def nickchanger(token, guild_Id):
     ...
     try:
          try:
               ...
               nick = requests.patch(f'https://canary.discordapp.com/api/v6/guilds/{guild_Id}/members/@me', headers=headers, json={'nick': str(nickname())}, proxies={"http" : "http://" + random.choice(proxies)})
          except Exception as e:
               try:
                log(e)
               finally:
...
def customnickchanger(token, guild_Id, custom):
     ...
     try:
          try:
               ...
               nick = requests.patch(f'https://canary.discordapp.com/api/v6/guilds/{guild_Id}/members/@me', headers=headers, json={'nick': custom}, proxies={"http" : "http://" + random.choice(proxies)})
          except Exception as e:
               try:
                log(e)
               finally:
...
def join(invite, token):
    ...
    try:
        try:
            ...
            a = requests.post(('https://discordapp.com/api/v9/invites/' + invite), headers=headers, proxies={"http" : "http://" + random.choice(proxies)})
    ...
    try:
        try:
            headers = {':authority':'canary.discord.com', 
             ...
             'x-debug-options':'bugReporterEnabled', 
             'x-super-properties':''}
            a = requests.patch('https://canary.discord.com/api/v9/users/@me', headers=headers, json={'bio': bio}, proxies={"http" : "http://" + random.choice(proxies)})
            if a.status_code == 200:
...
def leave(guild_id, token):
    ...
    try:
        try:
            ...
            a = requests.delete(('https://canary.discord.com/api/v9/users/@me/guilds/' + str(guild_id)), json=data, headers=headers, proxies={"http" : "http://" + random.choice(proxies)})
    ...
    try:
        ...
        for x in range(int(amount)):
            try:
                ...
                out = requests.post(f"https://discord.com/api/v9/channels/{str(channel_id)}/threads", headers=headers, json=data, proxies={"http" : "http://" + random.choice(proxies)})
...
def reaction(channel_id, message_id, addorrem, emoji_original, token):
    ...
    try:
        try:
            ...
            if addorrem == 'add':
                a = requests.put(f"https://discordapp.com/api/v6/channels/{channel_id}/messages/{message_id}/reactions/{emoji}/@me", headers=headers, proxies={"http" : "http://" + random.choice(proxies)})
                if a.status_code == 204:
                    log(colorama.Fore.WHITE + f"[+] Reaction {emoji_original} added! [{token}]")
                else:
                    log(colorama.Fore.RED + f"[-] Discord propably API banned this IP. Use a VPN. Error: {a.text} [{token}]")
            elif addorrem == 'rem':
                a = requests.delete(f"https://discordapp.com/api/v6/channels/{channel_id}/messages/{message_id}/reactions/{emoji}/@me", headers=headers, proxies={"http" : "http://" + random.choice(proxies)})
                if a.status_code == 204:
...
def friender(token, user):
    ...
    try:
        try:
            ...
            src = requests.post('https://canary.discordapp.com/api/v6/users/@me/relationships', headers=headers, json=payload, proxies={"http" : "http://" + random.choice(proxies)})
...
def dmspammer(token, userid, text, amount):
    ...
    try:
        ...
        src = requests.post('https://canary.discordapp.com/api/v6/users/@me/channels', headers=headers, json=payload, timeout=10, proxies={"http" : "http://" + random.choice(proxies)})
        dm_json = json.loads(src.content)
        payload = {'content':text,  'tts':False}
        for x in range(int(amount)):
            src = requests.post(f"https://canary.discordapp.com/api/v6/channels/{dm_json['id']}/messages", headers=headers, json=payload, timeout=10, proxies={"http" : "http://" + random.choice(proxies)})
            if src.status_code == 429:
                ratelimit = json.loads(src.content)
                time.sleep(float(ratelimit['retry_after'] / 1000))
            else:
...
def pfpchanger(token, img64):
    ...
    requests.patch('https://discord.com/api/v9/users/@me', headers=headers, json={"avatar": "data:image/png;base64," + img64}, proxies={"http" : "http://" + random.choice(proxies)})
...
def bypass_screening(invite_code, guild_id, token):
    ...
    try:
        try:
            ...
            requests.put(final_verif_url, headers=headers, json=data, proxies={"http" : "http://" + random.choice(proxies)})
        except Exception as e:
            try:
                log(e)
            finally:
...
def report(token, channel_id, guild_id, message_id, reason):
    ...
    payload = {'channel_id':channel_id, 
     ...
     'reason':reason}
    while True:
        r = requests.post('https://discord.com/api/v6/report', headers=headers, json=payload, proxies={"http" : "http://" + random.choice(proxies)})
        ...
        try:
            ...
            if hypesquad == 'bravery':
                ...
                houses = [
                 1, 2, 3]
                payload = {'house_id': random.choice(houses)}
            requests.post('https://discordapp.com/api/v6/hypesquad/online', headers=headers, json=payload, timeout=10, proxies={"http" : "http://" + random.choice(proxies)})
        except Exception as e:
            try:
                log(e)
            finally:
...
while True:
    ...
    if choice == '1':
        ...
        tokens = open('tokens.txt', 'r').read().splitlines()
        proxies = open('proxies.txt').read().splitlines()
        loading_print(f"Discord Username + Discriminator [example: BarbieDollGaming#6969]: ")
        user = input()
        for token in tokens:
